...
<button
  ...
  type="submit"
  class="space-x-2 transition-colors hover:bg-opacity-80 outline-none focus:ring ring-teal-500 mshadow-md  flex justify-center items-center
	{rounded == 'default'
    ? ' rounded-md'
    : rounded == 'full'
    ? ' rounded-full'
    ...
    {type == 'glass' && ' bg-opacity-25 text-white hover:bg-opacity-10'}
	  {color != 'default' ? `${color} text-white` : 'bg-gray-50'}
	  {size == 'xs'
    ? ' px-3 py-2 text-xs'
    : size == 'sm'
    ? ' px-4 py-2 text-sm'
